...
public class ArrayBi {
  ...
  /**
   * 1.Genera un array de tamaño n x m con números aleatorios cuyo intervalo
   ...
   * @return array de n filas x m columnas de números enteros generados
   * aleatoriamente
   */
  public static int[][] generaArrayBiInt(int n, int m, int min, int max) {
    ...
    int[][] x = new int[n][m];
    for (int i = 0; i < n; i++) {
      x[i] = array.Array.generaArrayInt(m, min, max);
    }
    return x;
   ...
   * @param i número de la fila a devolver
   * @return array unidimensional con la fila i-ésima del array x.
   */
   ...
   *
   * @param x, Array bidimensional de números enteros
   * @param j número de la columna a devolver
   * @return array unidimensional con la columna j-ésima del array x.
   */
  ...
  /**
   * 4.Devuelve la fila y la columna (en un array con dos elementos) de la
   ...
   *
   * @param x Array bidimensional de números enteros
   * @param , número entero, número a buscar en el array
   * @return array bidimensional con la fila y la columna donde está el número
   * buscado dentro del array x.
   */
  public static int[][] coordenadasEnArrayBiInt(int[][] x, int n) {
    ...
    int i = 0;
    do {
      p[0][1] = array.Array.posicionEnArray(n, columnaDeArrayBiInt(x, i));
      if (p[0][1] > -1) {
        p[0][0] = i;
      }
      i++;
  ...
  /**
   * 5.Dice si un número es o no punto de silla, es decir, mínimo en su fila y
   * máximo en su columna
   *
   * @param x Array bidimensional de números enteros
   * @param i número entero, posición i del número a comprobar si es punto de
   * silla
   * @param j número entero, posición j del número a comprobar si es punto de
   * silla
   * @return verdadero si es un punto de silla, falso si no es un número de
   * silla.
   */
  public static boolean esPuntoDeSilla(int[][] x, int i, int j) {

    return ((x[i][j] == array.Array.maximoArrayInt(columnaDeArrayBiInt(x, j)))
      && (x[i][j] == array.Array.minimoArrayInt(filaDeArrayBiInt(x, i))));

  }

  /**
   * 6.Devuelve un array que contiene una de las diagonales del array
   ...
   * @param d Cadena de caracteres, con los valores 'nose'(noroeste-sureste) o
   * 'neso' (noreste-suroeste)
   * @return array unidimensional con la diagonal que pasa por la fila y la
   * columna dadas y con la dirección dada.
   */
